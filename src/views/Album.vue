    ...
    <div class="container" style="padding-top: 1px;">
      ...
      <div class="songsheetlist">
        ...
        <div class="songsheet-listfor"
          ...
          <div class="songsheet-listitem odd" v-else>
            <div class="songsheet-songitem">
              <div class="number">
                <span>{{index + 1}}</span>
                <img @click="audioPlay(index)" class="hide" src="@/assets/play-blue.png" alt="">
...
export default {
  ...
  mounted() {
    ...
    let path = `http://127.0.0.1:3400/album?id=${id}`
    let path2= `http://127.0.0.1:3400/album/songs?id=${id}`
    let Getsongs = async (path) => {
      let res = await axios.get(path)
      let res2 = await axios.get(path2)
      this.songsheet = res.data.data
      this.songlist = res.data.data.songList
      //补充图片和url
      this.songlist.forEach(function(item, index) {
        item.album.picUrl = res2.data.data[index].picUrl
        item.url = res2.data.data[index].url
      })
    }
